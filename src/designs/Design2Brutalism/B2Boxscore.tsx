...
const B2Boxscore = () => {
  ...
  if (boxscoreQuery.isLoading || gameSummaryQuery.isLoading) {
    return (
      <div className={`brutal-container ${theme}`}>
        <div className="brutal-loading">Loading...</div>
      </div>
    );
  }
  ...
  if (boxscoreQuery.isError || !boxscoreQuery.data) {
    return (
      <div className={`brutal-container ${theme}`}>
        <div className="brutal-no-games">Error loading boxscore</div>
      </div>
    );
  }
  ...
  return (
    <div className={`brutal-container ${theme}`}>
      <header className="brutal-header">
        <Link to="/2" className="brutal-back-link">
          ← Back to Scores
        </Link>
        <DarkModeToggle />
      </header>

      <div className="brutal-boxscore-container">
        {/* Game Summary */}
        {summary && (
          <div className="brutal-game-summary">
            <div className="brutal-summary-grid">
              {/* Home Team */}
              <div className="brutal-summary-team">
                <div className="brutal-summary-team-info">
                  <img
                    src={getLogoUrl(summary.homeTeam.teamId)}
                    alt={summary.homeTeam.teamName}
                    className="brutal-summary-logo"
                    onError={(e) => {
                      e.currentTarget.src = placeholderTeamLogoUrl;
                    }}
                  />
                  <div className="brutal-summary-name">{summary.homeTeam.teamName}</div>
                </div>
                <div className={`brutal-summary-score ${homeWinning ? "winner" : ""}`}>
                  {summary.homeTeam.score}
                </div>
              </div>

              {/* Quarter Table */}
              <div className="brutal-quarter-wrapper">
                <table className="brutal-quarter-table">
                  <thead>
                    <tr>
                      <th></th>
                      <th>1</th>
              ...
              <div className="brutal-summary-team">
                <div className="brutal-summary-team-info">
                  ...
                  />
                  <div className="brutal-summary-name">{summary.awayTeam.teamName}</div>
                </div>
                <div className={`brutal-summary-score ${awayWinning ? "winner" : ""}`}>
        ...
        {/* Player Tables */}
        <B2PlayerTable team={game.homeTeam} />
        <B2PlayerTable team={game.awayTeam} />
      </div>
    </div>
  );
};
...
const B2PlayerTable = ({team}: PlayerTableProps) => {
  const activePlayers = team.players.filter((player: Player) => player.comment === "");

  return (
    <div className="brutal-player-section">
      <h2 className="brutal-player-header">{team.teamName}</h2>
      <div style={{overflowX: "auto"}}>
        <table className="brutal-player-table">
          <thead>
            <tr>
              <th style={{minWidth: "100px"}}>Player</th>
              <th>Pts</th>
              <th>Reb</th>
              <th>Ast</th>
              <th>Stl</th>
            ...
            {activePlayers.map((player: Player) => {
              ...
              return (
                <tr key={player.personId}>
                  <td>{player.nameI}</td>
                  <td className={isHighPoints ? "brutal-stat-highlight" : ""}>
                    {stats.points}
                  </td>
                  <td className={isHighRebounds ? "brutal-stat-highlight" : ""}>
                    {stats.reboundsTotal}
                  </td>
                  <td className={isHighAssists ? "brutal-stat-highlight" : ""}>
                    {stats.assists}
                  </td>
                  <td>{stats.steals}</td>
                  <td>{stats.blocks}</td>
