...
const C4Boxscore = () => {
  ...
  if (boxscoreQuery.isLoading || gameSummaryQuery.isLoading) {
    return (
      <div className={`cyber-container ${theme}`}>
        <div className="cyber-loading">Loading Data Stream...</div>
      </div>
    );
  }
  ...
  if (boxscoreQuery.isError || !boxscoreQuery.data) {
    return (
      <div className={`cyber-container ${theme}`}>
        <div className="cyber-no-games">Data Stream Error</div>
      </div>
    );
  }
  ...
  return (
    <div className={`cyber-container ${theme}`}>
      <Link to="/4" className="cyber-back-link">
        ← Back to Grid
      </Link>
      ...
      <div className="cyber-boxscore-container" style={{paddingTop: "4rem"}}>
        {/* Game Summary */}
        {summary && (
          <div className="cyber-game-summary">
            <div className="cyber-summary-grid">
              {/* Home Team */}
              <div className="cyber-summary-team">
                <img
                  src={getLogoUrl(summary.homeTeam.teamId)}
                  alt={summary.homeTeam.teamName}
                  className="cyber-summary-logo"
                  onError={(e) => {
                    e.currentTarget.src = placeholderTeamLogoUrl;
                  }}
                />
                <div className="cyber-summary-name">{summary.homeTeam.teamName}</div>
                <div className={`cyber-summary-score ${homeWinning ? "winner" : ""}`}>
                  {summary.homeTeam.score}
                </div>
              </div>

              {/* Quarter Table */}
              <div className="cyber-quarter-wrapper">
                <table className="cyber-quarter-table">
                  <thead>
                    <tr>
                      <th></th>
                      <th>Q1</th>
              ...
              <div className="cyber-summary-team">
                ...
                />
                <div className="cyber-summary-name">{summary.awayTeam.teamName}</div>
                <div className={`cyber-summary-score ${awayWinning ? "winner" : ""}`}>
        ...
        {/* Player Tables */}
        <C4PlayerTable team={game.homeTeam} />
        <C4PlayerTable team={game.awayTeam} />
      </div>
    </div>
  );
};
...
const C4PlayerTable = ({team}: PlayerTableProps) => {
  const activePlayers = team.players.filter((player: Player) => player.comment === "");

  return (
    <div className="cyber-player-section">
      <h2 className="cyber-player-header">{team.teamName}</h2>
      <div style={{overflowX: "auto"}}>
        <table className="cyber-player-table">
          <thead>
            <tr>
            ...
            {activePlayers.map((player: Player) => {
              ...
              return (
                <tr key={player.personId}>
                  <td>{player.nameI}</td>
                  <td className={isHighPoints ? "cyber-stat-highlight" : ""}>
                    {stats.points}
                  </td>
                  <td className={isHighRebounds ? "cyber-stat-highlight" : ""}>
                    {stats.reboundsTotal}
                  </td>
                  <td className={isHighAssists ? "cyber-stat-highlight" : ""}>
                    {stats.assists}
                  </td>
                  <td>{stats.steals}</td>
                  <td>{stats.blocks}</td>
